...
const Footer = ({ data, icons }) => {
  ...
  return (
    <div className={`${showCookieBanner ? "mb-16" : ""}`}>
      <div className={`flex justify-center items-center w-full bg-[#f7f7f7]`}>
        <div className="max-w-[1600px] w-full sm:mx-8 mx-4 my-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-y-10 gap-x-4">
          ...
          <div className="sm:order-2 lg:order-4">
            <SocialLinks data={icons?.social_links} />
          </div>
        </div>
      </div>
      <div
        className={`flex justify-center items-center w-full bg-white border-t-2 border-[#81818133]`}
      >
        <div className="max-w-[1600px] w-full sm:mx-8 mx-4 my-6">
          <p className="text-sm font-lato text-[#767676]">{data?.copyright}</p>
        </div>
      </div>
    </div>
  );
