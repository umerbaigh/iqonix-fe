...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    description: `Department ${resp?.data[0]?.attributes?.name}`,
  };
}
...
const Page = async ({ params, searchParams }) => {
  ...
  const { department } = await params;
  const urls = {
    department: `/departments/?filters[slug][$eq]=${department}`,
  };
  let url = `/get-department/${department}/?page=${page}&pageSize=35`;
  let url2 = `/get-department/${department}/?page=-1`;
  let sortOption = "";
  if (order === "date") {
  ...
  const [departments, pageProducts, allProducts] = await Promise.all([
  ...
  const breadcrumbs = [
    ...
    {
      title: `product department ${departments?.data[0]?.attributes?.name}`,
    },
  ];
  ...
  return (
    <div>
      <Layout>
        ...
        <Products
          departmentName={departments?.data[0]?.attributes?.name}
          allProducts={allProducts}
          products={pageProducts?.products}
          totalProducts={length}
          description={departments?.data[0]?.attributes?.description}
          isSearch={true}
        />
      </Layout>
    </div>
