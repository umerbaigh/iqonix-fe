...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    description: `Shop ${resp?.data[0]?.attributes?.name}`,
  };
}
...
const Page = async ({ params, searchParams }) => {
  ...
  let url = `/get-shop/${shop}/?page=${page}&pageSize=35`;
  let url2 = `/get-shop/${shop}/?page=-1`;
  let sortOption = "";
  if (order === "date") {
  ...
  const [shops, pageProducts, allProducts] = await Promise.all([
  ...
  const breadcrumbs = [
    ...
    {
      title: `product shops ${shops?.data[0]?.attributes?.name}`,
    },
  ];
  ...
  return (
    <div>
      <Layout>
        ...
        <Products
          departmentName={shops?.data[0]?.attributes?.name}
